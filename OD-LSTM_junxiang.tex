...
We first display the experiment settings within our work.
...
\textbf{Character Embedding}. In order to compare with Lattice LSTM (Zhang and Yang 2018) and Lexicon Rethinking CNN (Gui et al. 2019), we use the same character embedding provided in their papers (these two paper use the same character embedding and word embedding). 
...
For compared purposed, we impletement below models:

\textbf{Character LSTM}. We basicly impletement the character-level LSTM as the first model, no any other properties used (like bichar, softword, position etc.). We abbreviate it as C-LSTM.

\textbf{Stanford NER model}. Stanford NER tool is used in our work to do comparision, for it is convenient to add handcrafted features into model. And it can use dictionary (called gazette in its term) directly, which can be compared to our OD model intuitively. For short, denote it as S-NER. 
...
\textbf{Tag Embedding LSTM}. This is the model which we proposed in this paper. We denote it as TE-LSTM.
